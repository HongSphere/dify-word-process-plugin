...
class WordExtractorTool(Tool):
    """Extract text and images from both legacy .doc and newer .docx Word files."""
    ...
    def _invoke(
        ...
        filename = input_file.filename or "document"
        extension = (input_file.extension or "").lower().lstrip(".")
        blob = input_file.blob or b""
        if not blob:
            raise ValueError("Uploaded file is empty.")

        # Determine format based on magic number and/or extension
        is_doc = False
        is_docx = False
        ...
        if olefile.isOleFile(io.BytesIO(blob)):
            ...
            # Fallback to extension if validation fails
            if extension == "doc" or filename.lower().endswith(".doc"):
                is_doc = True
            elif extension == "docx" or filename.lower().endswith(".docx"):
                is_docx = True

        if is_doc:
            yield from self._process_doc(blob, filename)
        elif is_docx:
            yield from self._process_docx(blob, filename)
        else:
            raise ValueError("Unsupported file format. Only .doc and .docx files are supported.")

    def _process_doc(self, blob: bytes, filename: str) -> Generator[ToolInvokeMessage, None, None]:
        try:
            result = self._extract_doc(blob)
        except ValueError as e:
            raise ValueError(f"{e}")
        except Exception as e:
            raise Exception(f"Failed to parse DOC file: {e}")
    ...
    def _process_docx(self, blob: bytes, filename: str) -> Generator[ToolInvokeMessage, None, None]:
        try:
                    ...
                    if image_files:
                        ...
                        for idx, image_path in enumerate(image_files, start=1):
                            try:
                                ...
                                base_filename = re.sub(r"\.docx$", "", filename, flags=re.IGNORECASE) or "document"
                                output_filename = f"{base_filename}_image_{idx}{file_ext}"
